...
wget https://github.com/yeqing112/webSiteConfig/archive/1.0.0.tar.gz \
&& tar -zxvf 1.0.0.tar.gz \
&& cd webSiteConfig-1.0.0
mkdir data
# 创建容器网络
...
# 启动nginx
docker run -d --name nginx -p 80:80 -p 443:443 -v $PWD/www:/data/www -v $PWD/nginx/conf.d:/etc/nginx/conf.d -v $PWD/nginx/fastcgi.conf:/etc/nginx/fastcgi.conf -v $PWD/ssl:/etc/nginx/ssl --network=myproxy --restart=always registry.cn-shanghai.aliyuncs.com/yeqing112/nginx:1.19.1
